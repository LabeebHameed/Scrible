...
export class ReminderScheduler {
  ...
  async tick(now = Date.now()): Promise<number> {
    ...
    for (const trigger of due) {
      ...
      // Check if user has an active device with verified local exact alarms capability
      const userDevices = (await this.db
        .prepare('SELECT capabilities FROM devices WHERE user_id = ?')
        .all(userId)) as Array<{ capabilities?: string }>;

      const hasExactAlarmDevice = userDevices.some((d) => {
        try {
          const caps = JSON.parse(String(d.capabilities ?? '{}')) as { exactAlarms?: boolean };
          return caps.exactAlarms === true;
        } catch {
          return false;
        }
      });
      ...
      // If local alarms are verified on device, initial delivery is handled locally (no double alert).
      // Server push still handles subsequent re-nags or devices without verified exact alarms.
      let sent = false;
      if (!isFirstDelivery || !hasExactAlarmDevice) {
        sent = await this.dispatcher.notify(
          userId,
