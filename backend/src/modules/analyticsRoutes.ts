...
export function registerAnalyticsRoutes(app: FastifyInstance, db: Db, forwarder: AnalyticsForwarder): void {
  ...
  app.post('/v1/analytics/events', { preHandler: app.authenticate }, async (req, reply) => {
    ...
    for (const event of (events ?? []).slice(0, 100)) {
      ...
      const status = await forwarder.track(req.userId, event.name, event.props ?? {});
  ...
  app.get('/v1/me/export', { preHandler: app.authenticate }, async (req) => {
    ...
    const all = async (table: string) =>
      (await db.prepare(`SELECT * FROM ${table} WHERE user_id = ?`).all(userId)) as Array<Record<string, unknown>>;

    const user = (await db
      .prepare('SELECT id, email, timezone, working_hours, notification_prefs, created_at FROM users WHERE id = ?')
      .get(userId)) as Record<string, unknown>;
    const items = (await all('items')).map(rowToItem);
    const subtasks = (await all('subtasks')).map(rowToSubtask);
    const profileRow = (await db.prepare('SELECT attributes, sources, storage, updated_at FROM profiles WHERE user_id = ?').get(userId)) as
      | Record<string, unknown>
      | undefined;
    const pseudo = (await db.prepare('SELECT pseudo_id FROM analytics_ids WHERE user_id = ?').get(userId)) as
      | { pseudo_id: string }
      | undefined;
    ...
    return {
      ...
      consents: await currentConsents(db, userId),
      items,
      subtasks,
      scheduleBlocks: await all('schedule_blocks'),
      reminders: await all('reminder_triggers'),
      calendarLinks: (await all('calendar_links')).map((l) => ({
        id: l.id,
        provider: l.provider,
        accountId: l.account_id,
        // OAuth token material is a credential, not user content — never exported.
      })),
      devices: (await all('devices')).map((d) => ({ id: d.id, platform: d.platform, lastSeen: d.last_seen })),
      ...
      profile: profileRow
        ...
        : null,
      importJobs: (await all('import_jobs')).map((j) => ({ id: j.id, source: j.source, state: j.state, rawDeletedAt: j.deleted_at, createdAt: j.created_at })),
      analytics: pseudo
        ? {
