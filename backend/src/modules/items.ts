...
export function registerItems(app: FastifyInstance, db: Db, sync: SyncEngine): void {
  ...
  app.get('/v1/items/:id', { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const item = sync.itemById(req.userId, id);
    if (!item) return reply.code(404).send({ error: 'not found' });
    item.subtasks = sync.subtasksFor(req.userId, id);
    return item;
  });
  ...
  app.get('/v1/queue', { preHandler: app.authenticate }, async (req) => {
    ...
    for (const item of queue) item.subtasks = sync.subtasksFor(req.userId, item.id);
    return queue;
  });
  ...
  app.post('/v1/items', { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const results = sync.applyOps(req.userId, [
      {
    ...
    const item = sync.itemById(req.userId, id) as Item;
    item.subtasks = sync.subtasksFor(req.userId, id);
  ...
  const restOp =
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      return { ok: true, status, item: sync.itemById(req.userId, id) };
    };
