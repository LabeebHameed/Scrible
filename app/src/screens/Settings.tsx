...
import { type AlarmPermissionStatus, checkAlarmPermissions, openAlarmSettings } from '../alarms';
...
export function SettingsScreen(props: {
  ...
  const [alarmStatus, setAlarmStatus] = useState<AlarmPermissionStatus | null>(null);
  ...
  const refresh = async () => {
    try {
      ...
      setAlarmStatus(await checkAlarmPermissions());
      setError(null);
    } catch (err) {
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={{ padding: 18 }}>
      ...
      <Text style={styles.section}>Alarms & System Permissions</Text>
      <View style={styles.row}>
        <View style={{ flex: 1 }}>
          <Text style={styles.label}>
            {alarmStatus?.canScheduleExact ? 'Exact alarms on' : 'Exact alarms disabled'}
          </Text>
          <Text style={styles.detail}>
            {alarmStatus?.canScheduleExact
              ? 'Local wall-clock alarms ring deterministically even when app is closed or offline.'
              : 'Server push fallback active. Grant permission for offline exact alarms.'}
          </Text>
        </View>
        {!alarmStatus?.canScheduleExact ? (
          <Pressable style={styles.buttonGhost} onPress={() => void openAlarmSettings()}>
            <Text style={styles.buttonGhostText}>Enable</Text>
          </Pressable>
        ) : null}
      </View>
