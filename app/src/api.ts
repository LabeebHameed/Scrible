...
export interface ApiClient {
  ...
  registerDevice(platform: string, pushToken: string | null, capabilities?: Record<string, unknown>): Promise<void>;
...
export class HttpApi implements ApiClient {
  ...
  async registerDevice(platform: string, pushToken: string | null, capabilities?: Record<string, unknown>) {
    await this.req('POST', '/v1/devices', { platform, pushToken, capabilities });
  }
  async markReminderSeen(reminderId: string) {
    await this.req('POST', `/v1/reminders/${reminderId}/seen`);
  }
  async snoozeReminder(reminderId: string, minutes: number = 10) {
